...
// file section of UI
const mjuiDef defFile[] = {{mjITEM_SECTION, "File", mjPRESERVE, nullptr, "AF"}, {mjITEM_BUTTON, "Save xml", 2, nullptr, ""},
...
void UpdateProfiler(mj::Simulate* sim, const mjModel* m, const mjData* d) {
  ...
  if (number) {  // skip update if no measurements
    float tdata[5] = {static_cast<float>(total / number), static_cast<float>(d->timer[mjTIMER_POS_COLLISION].duration / number),
                      static_cast<float>(d->timer[mjTIMER_POS_MAKE].duration / number) + static_cast<float>(d->timer[mjTIMER_POS_PROJECT].duration / number),
...
static void mjui0_update_section(mj::Simulate* sim, int section) { mjui_update(section, -1, &sim->ui0, &sim->uistate, &sim->platform_ui->mjr_context()); }

// prepare info text
void UpdateInfoText(mj::Simulate* sim, const mjModel* m, const mjData* d, char (&title)[mj::Simulate::kMaxFilenameLength],
  ...
  // add Energy if enabled
  {
    ...
    if (mjENABLED(mjENBL_FWDINV)) {
      mju::sprintf_arr(tmp, "\n%.1f %.1f", mju_log10(mju_max(mjMINVAL, d->solver_fwdinv[0])), mju_log10(mju_max(mjMINVAL, d->solver_fwdinv[1])));
      mju::strcat_arr(content, tmp);
      mju::strcat_arr(title, "\nFwdInv");
    }
...
void MakePhysicsSection(mj::Simulate* sim) {
  ...
  mjuiDef defOverride[] = {{mjITEM_SEPARATOR, "Contact Override", mjPRESERVE},       {mjITEM_EDITNUM, "Margin", 2, &(opt->o_margin), "1"},
                           {mjITEM_EDITNUM, "Sol Imp", 2, &(opt->o_solimp), "5"},    {mjITEM_EDITNUM, "Sol Ref", 2, &(opt->o_solref), "2"},
                           {mjITEM_EDITNUM, "Friction", 2, &(opt->o_friction), "5"}, {mjITEM_END}};
  mjuiDef defDisableActuator[] = {
      {mjITEM_SEPARATOR, "Actuator Group Enable", mjPRESERVE},           {mjITEM_CHECKBYTE, "Act Group 0", 2, sim->enableactuator + 0, ""},
      {mjITEM_CHECKBYTE, "Act Group 1", 2, sim->enableactuator + 1, ""}, {mjITEM_CHECKBYTE, "Act Group 2", 2, sim->enableactuator + 2, ""},
      {mjITEM_CHECKBYTE, "Act Group 3", 2, sim->enableactuator + 3, ""}, {mjITEM_CHECKBYTE, "Act Group 4", 2, sim->enableactuator + 4, ""},
      {mjITEM_CHECKBYTE, "Act Group 5", 2, sim->enableactuator + 5, ""}, {mjITEM_END}};
...
void MakeRenderingSection(mj::Simulate* sim, const mjModel* m) {
  ...
  // create tree slider
  mjuiDef defTree[] = {
      {mjITEM_SLIDERINT, "Tree depth", 2, &sim->opt.bvh_depth, "0 20"}, {mjITEM_SLIDERINT, "Flex layer", 2, &sim->opt.flex_layer, "0 10"}, {mjITEM_END}};
...
void MakeControlSection(mj::Simulate* sim) {
  mjuiDef defControl[] = {{mjITEM_SECTION, "Control", mjPRESERVE, nullptr, "AC"}, {mjITEM_BUTTON, "Clear all", 2}, {mjITEM_END}};
  mjuiDef defSlider[] = {{mjITEM_SLIDERNUM, "", 2, nullptr, "0 1"}, {mjITEM_END}};
  ...
  for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++) {
    ...
    if (sim->actuator_ctrlrange_[i].has_value())
      mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", sim->actuator_ctrlrange_[i]->first, sim->actuator_ctrlrange_[i]->second);
    else {
      mju::strcpy_arr(defSlider[0].other, "-1 1");
    }
...
void CopyCamera(mj::Simulate* sim) {
  ...
  mju::sprintf_arr(clipboard, "<camera pos=\"%.3f %.3f %.3f\" xyaxes=\"%.3f %.3f %.3f %.3f %.3f %.3f\"/>\n", (camera[0].pos[0] + camera[1].pos[0]) / 2,
                   (camera[0].pos[1] + camera[1].pos[1]) / 2, (camera[0].pos[2] + camera[1].pos[2]) / 2, cam_right[0], cam_right[1], cam_right[2],
...
void UiModify(mjUI* ui, mjuiState* state, mjrContext* con) {
  ...
  int id = ui->auxid;
  if (con->auxFBO[id] == 0 || con->auxFBO_r[id] == 0 || con->auxColor[id] == 0 || con->auxColor_r[id] == 0 || con->auxWidth[id] != ui->width ||
...
void UiEvent(mjuiState* state) {
  ...
  if ((state->dragrect == sim->ui0.rectid) || (state->dragrect == 0 && state->mouserect == sim->ui0.rectid) || state->type == mjEVENT_KEY) {
    // process UI event
  ...
  if ((state->dragrect == sim->ui1.rectid) || (state->dragrect == 0 && state->mouserect == sim->ui1.rectid) || state->type == mjEVENT_KEY) {
    // process UI event
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (sim->m_ || sim->is_passive_)) {
    ...
    if (sim->pert.active) {
      if (!sim->is_passive_) {
        ...
        mjv_movePerturbFromState(&sim->scnstate_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->pert);
      }
    } else {
      if (!sim->is_passive_) {
        mjv_moveCamera(sim->m_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->cam);
      } else {
        mjv_moveCameraFromState(&sim->scnstate_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->cam);
      }
    }
    return;
  }
...
Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui, mjvCamera* cam, mjvOption* opt, mjvPerturb* pert, bool is_passive)
    : is_passive_(is_passive), cam(*cam), opt(*opt), pert(*pert), platform_ui(std::move(platform_ui)), uistate(this->platform_ui->state()) {
...
#undef X
  ...
  if (pending_.reset) {
    mj_resetData(m_, d_);
    mju_copy(d_->qpos, m_->key_qpos, m_->nq * 1);
    mj_forward(m_, d_);
    load_error[0] = '\0';
  ...
  if (pending_.select) {
    ...
    int selbody = mjv_select(m_, d_, &this->opt, static_cast<mjtNum>(r.width) / r.height, (pending_.select_state.x - r.left) / r.width,
...
void Simulate::LoadOnRenderThread() {
  ...
  for (int i = 0; i < this->m_->nu; ++i) {
    if (this->m_->actuator_ctrllimited[i]) {
      actuator_ctrlrange_.push_back(std::make_pair(this->m_->actuator_ctrlrange[2 * i], this->m_->actuator_ctrlrange[2 * i + 1]));
    } else {
...
void Simulate::Render() {
  ...
  if (is_passive_) {
    if (this->ui0_enable && this->ui0.sect[SECT_RENDERING].state &&
        (cam_prev_.type != cam.type || cam_prev_.fixedcamid != cam.fixedcamid || cam_prev_.trackbodyid != cam.trackbodyid || opt_prev_.label != opt.label ||
  ...
  // show info
  if (this->info) {
    mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect, this->info_title, this->info_content, &this->platform_ui->mjr_context());
  }
...
void Simulate::RenderLoop() {
  ...
  // run event loop
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load() && RuntimeFlags::instance().is_running()) {
    {
      ...
      if (!this->is_passive_) {
        ...
        scnstate_.data.warning[mjWARN_VGEOMFULL].number += mjv_updateSceneFromState(&scnstate_, &this->opt, &this->pert, &this->cam, mjCAT_ALL, &this->scn);
      }
