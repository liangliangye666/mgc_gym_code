...
const double simRefreshFraction =
    0.007;                      // fraction of refresh available for simulation
...
void MujocoSimulator(int argc, char** argv) {
  ...
  std::string path = workspacePath + "/sim/model/l4a/xml/l4aurdf20241119.xml";
  ...
  const mjModel* mj_model = LoadModel(filename, *sim);
  const mjData* mj_data = mj_makeData(mj_model);

  for (int i = 0; i < mj_model->njnt; ++i) {
    const char* joint_name = mj_id2name(mj_model, mjOBJ_JOINT, i);
    if (joint_name) {
      std::cout << "Joint index: " << i << ", name: " << joint_name
                << std::endl;
    } else {
      std::cout << "Joint index: " << i << ", name: [Unnamed]" << std::endl;
    }
  }

  std::cout << "mj_model->nq: " << mj_model->nq << std::endl;
  for (size_t i = 0; i < mj_model->nq; i++) {
    std::cout << i << ": \t" << mj_data->qpos[i] << std::endl;
  }
