...
// Load parameters
  ...
  config_ = YAML::LoadFile(workspacePath + "/platforms/l5b/control/rl_parameters_sim.yaml");
#elif PHYSICS_ENABLE
  std::string workspacePath = std::getenv("PROJECT_ROOT_DIR");
  config_ = YAML::LoadFile(workspacePath + "/platforms/l5b/control/rl_parameters_physics.yaml");
#endif
  LoadParameters();
  pd_controller_joints_ = PdController<Eigen::VectorXd>(kp_joints_, kd_joints_);
  auto start = std::chrono::high_resolution_clock::now();
  controller_ = torch::jit::load(workspacePath + "/platforms/l5b/control/module/" + model_ctrl_);
  estimator_ = torch::jit::load(workspacePath + "/platforms/l5b/control/module/" + model_est_);
  WarmUpModels();
