...
void standMode_step(standmode_output_t* standmode_output, standmode_input_t* standmode_input) {
  ...
  if(robot_model.vel_x_des_ >= 0.2){
    robot_model.distance_ = 1.0;
  }else{
    if(robot_model.distance_ >= 0.001){
      robot_model.distance_ -= 0.001;
    }
  }
...
if (std::abs(standmode_input->Handle_signals.a1 - 0.5) <= 0.1 || handle_LB) {
    ...
    ctlSigs->vel_x_des = (standmode_input->Handle_signals.a1 - 0.5) * (-0.6) * handle_LT;
  }
  ...
  if (handle_RT) {
    ...
    ctlSigs->gait_phase = std::fmod(ctlSigs->gait_counter * 0.005, 0.6) / 0.6;
