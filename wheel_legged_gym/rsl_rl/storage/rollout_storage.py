...
class RolloutStorage:
    ...
    def compute_returns(self, last_values, gamma, lam):
        advantage = 0 # 初始化优势函数
        for step in reversed(range(self.num_transitions_per_env)): # 反向遍历时间步,从最后一个时间步向前计算到第一个
            if step == self.num_transitions_per_env - 1: # 对下一个状态的价值函数赋值,其实就是下一个状态的价值V(t+1)
                next_values = last_values
            else:
                next_values = self.values[step + 1]
            next_is_not_terminal = 1.0 - self.dones[step].float() # 确定下一个是否终点
            delta = ( # 先算出各个步的TD_error
                self.rewards[step] # 即时奖励
                + next_is_not_terminal * gamma * next_values # 后续状态估计价值
                - self.values[step] # 当前状态估计价值
            )
            advantage = delta + next_is_not_terminal * gamma * lam * advantage # 套用GAE公式计算优势函数,此公式=sum(gamma*lambda)^l*delta_(t+l),即GAE公式
            self.returns[step] = advantage + self.values[step] # 当前状态的回报 = 优势 + 当前状态的价值估计

        # Compute and normalize the advantages
        self.advantages = self.returns - self.values # 获取所有步的优势
        self.advantages = (self.advantages - self.advantages.mean()) / ( # 优势函数归一化处理
